...
4) Configure STACK (optional but recommended):
   - `./init/scripts/stack-init.sh`
5) Open `http://localhost:${MOODLE_HTTP_PORT}` and log in with your admin credentials.
...
- `MOODLE_ADMIN_FORCE_PASSWORD_CHANGE` (`1` forces an admin password change at
  first login; set in the `.env` generated by `tools/start.sh`)
